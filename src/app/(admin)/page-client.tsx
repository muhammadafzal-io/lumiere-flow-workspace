...
function triggerDescription(rule: any): string {
  ...
  switch (rule.trigger_type) {
    case "Birthday":       return `${cfg.days_before ?? "?"} days before birthday`;
    case "Inactivity":     return `No visit in ${cfg.days ?? "?"} days`;
    case "Treatment-based":return `${cfg.days_after ?? "?"} days after ${cfg.treatment ?? "treatment"}`;
    case "Date-based":     return `One-time on ${cfg.date ?? "?"}`;
    case "No-show recovery":return `${cfg.hours_after ?? "?"}h after missed appointment`;
...
export default function Dashboard() {
  ...
  const weekDelta =
    m && m.appointmentsLastWeek > 0
      ? Math.round(((m.appointmentsThisWeek - m.appointmentsLastWeek) / m.appointmentsLastWeek) * 100)
    ...
    <div className="space-y-6">
      ...
      {error && (
        <div className="rounded-lg border border-destructive/30 bg-destructive/5 px-4 py-3 text-sm text-destructive">
          ...
          <button onClick={fetchDashboard} className="underline">Retry</button>
        </div>
      )}
              ...
              : activeRules.length === 0
                ? (
                    <p className="text-sm text-muted-foreground text-center py-4">
                      No active rules. <Link href="/rules" className="text-primary hover:underline">Create one →</Link>
                    </p>
                  )
      ...
      <div className="rounded-lg border bg-card">
        <div className="px-5 py-4 border-b flex items-center justify-between">
          ...
          <Link href="/calendar" className="text-xs text-primary hover:underline inline-flex items-center gap-1">
        ...
        <div className="grid grid-cols-7 divide-x">
          {next7.map((day) => {
            ...
            const items = loading ? [] : eventsOnDay(events, day).sort((a, b) =>
              ...
              >
                ...
                <div className="space-y-1.5 flex-1">
                  {items.slice(0, 3).map((e) => (
                    ...
                    >
                      <div className="text-[10px] font-semibold text-foreground">{fmtTime(e.startTime)}</div>
                      <div className="text-[11px] truncate">{e.clientName.split(" ")[0] || "Client"}</div>
                      <div className="text-[10px] text-muted-foreground truncate">{e.treatment}</div>
                    </Link>
                  ))}
                  {items.length > 3 && (
                    <Link href="/calendar" className="block text-[10px] text-primary hover:underline">
                      +{items.length - 3} more
                    </Link>
                  )}
                </div>
      ...
      <div className="rounded-lg border bg-card">
        <div className="px-5 py-4 border-b flex items-center justify-between">
          ...
          <Link href="/rules" className="text-xs text-primary hover:underline">Manage rules</Link>
        </div>
              ...
              {loading
                ? Array.from({ length: 3 }).map((_, i) => (
                    <tr key={i} className="animate-pulse">
                      <td className="px-5 py-3"><div className="h-3 bg-muted rounded w-32" /></td>
                      <td className="px-5 py-3"><div className="h-3 bg-muted rounded w-28" /></td>
                      <td className="px-5 py-3"><div className="h-3 bg-muted rounded w-16" /></td>
                      <td className="px-5 py-3"><div className="h-3 bg-muted rounded w-12 ml-auto" /></td>
                    </tr>
                  ))
                : activeRules.length === 0
                  ? (
                      <tr>
                        <td colSpan={4} className="px-5 py-8 text-center text-sm text-muted-foreground">
                          No active rules yet.{" "}
                          <Link href="/rules" className="text-primary hover:underline">Create one →</Link>
                        </td>
                      </tr>
                    )
                  : activeRules.map((r: any) => (
