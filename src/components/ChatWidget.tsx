      ...
      <div className="border-t bg-card px-4 py-3 flex-shrink-0 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
        {composerReady ? (
          <div className="flex items-end gap-2">
            {/* A 2px moving colour edge appears around the box while there is text in it. */}
            <div
              className={`flex-1 rounded-[0.85rem] p-[2px] transition-all ${
                input.trim() ? "chat-composer-live" : "bg-transparent"
              }`}
            >
              <textarea
                ref={textareaRef}
                ...
                className="block w-full resize-none bg-muted rounded-xl px-4 py-2.5 text-sm text-foreground placeholder-muted-foreground focus:outline-none transition-all max-h-32 overflow-y-auto"
            ...
            <button
              ...
              className={`w-10 h-10 rounded-xl bg-primary flex items-center justify-center flex-shrink-0 transition-all hover:bg-primary/90 disabled:opacity-40 disabled:cursor-not-allowed ${input.trim() && !loading ? "chat-send-live" : ""}`}
